...
from services import verificar_login, verificar_expiracao_senha, alterar_senha_usuario, desbloquear_usuario, verificar_grupo
from config import *
...
def login():
    ...
    if request.method == 'POST':
        ...
        grupo = os.getenv('grupo_dn')
        
        if verificar_login(username, password) and verificar_grupo(username, grupo):
            session['username'] = username
            flash('Login realizado com sucesso!', 'success')
            return redirect(url_for('index'))
        else:
            flash('Nome de usuário, senha inválidos ou usuário não pertence ao grupo.', 'error')
